...
class DiagnosticsRunner:
    ...
    def run_all(self):
        ...
        self.plot_flux_ratio_linear_envelop()
        self.plot_flux_ratio_quadratic_envelop()
        self.plot_error_validation()
        self.plot_source_counts()
        self.plot_size_snr()
        print(f"Diagnostics saved to: {self.out_dir}")

    def plot_flux_ratio_linear_envelop(self):
        df = self.df
        ...
        ax.set_title('Metric 1a: Resolved Source Check - Linear Envelop')
        ax.legend()
        
        plt.savefig(os.path.join(self.out_dir, "diag_flux_ratio_linear_envelop.png"), dpi=300, bbox_inches='tight')
        plt.close()
        
    def plot_flux_ratio_quadratic_envelop(self):
        df = self.df
        ratio = df['Total_flux'] / df['Peak_flux']
        snr = df['SNR']
        ...
        # --- FIX STARTS HERE ---
        # Matches Text Eq 15 (Quadratic sum of errors)
        # 0.05 represents ~5% systematic/calibration uncertainty
        delta_sys = 0.05 
        envelope = np.sqrt((2.5 / x)**2 + delta_sys**2)
        
        upper = 1 + envelope
        lower = 1 - envelope
        # --- FIX ENDS HERE ---
        
        ax.plot(x, upper, 'r--', label=r'Unresolved Envelope')
        ax.plot(x, lower, 'r--')
        ax.axhline(1.0, color='gray', linestyle='-', alpha=0.5)
        ...
        ax.set_xlabel('Signal-to-Noise Ratio (SNR)')
        ax.set_ylabel(r'Flux Ratio ($S_{int} / S_{peak}$)')
        ax.set_title('Metric 1b: Resolved Source Check - Quadratic Sum of Errors Envelope')
        ax.legend()
        
        plt.savefig(os.path.join(self.out_dir, "diag_flux_ratio_quadratic_sum_error_envelop.png"), dpi=300, bbox_inches='tight')
        plt.close()
